...
## 对于AI推测经历：
1. 总共生成6个AI推测经历卡片（每个方向2个，共3个方向）
2. 重点关注兴趣、活动等非结构化经历
3. 必须与目标方向高度相关且具体可信
4. 包含详细的灰色提示文本，指导用户应该输入什么内容
5. 推测经历要具体到场景、互动对象、成果产出
6. 避免笼统描述，要有可感知的细节
7. AI推测经历的内容字段应为占位符格式，用户需要编辑替换
...
特别注意：真实经历宁可生成少量准确的卡片，也不要编造虚假信息！AI推测经历要基于目标方向的典型行为模式，具体且可信。
`;
